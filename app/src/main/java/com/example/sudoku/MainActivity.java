...
public class MainActivity extends AppCompatActivity {
	...
	private List<List<TextView>> textCells;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initiateWidgets();
		InputButtonHandler.initialise(buttonsNumbers, buttonDelete);
		GameHandler.initialise(this, textCells);
	...
	private void initiateWidgets() {
		...
		for (int[] IDRows : ID_CELLS) {
			List<TextView> rowList = new ArrayList<>();
			for (int ID : IDRows) {
				rowList.add(findViewById(ID));
			}
			textCells.add(rowList);
		}
